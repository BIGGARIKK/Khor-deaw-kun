...
const BannerColorModal = ({ setIsColorModalOpen, bannerColor, setBannerColor, bannerPresets }) => {
  ...
  const colorPresets = [
    ...
    { id: 2, color: '#6de6e6', pattern: 'radial-gradient(rgba(255,255,255,0.6) 15%, transparent 16%), radial-gradient(rgba(255,255,255,0.6) 15%, transparent 16%)', size: '20px 20px', position: '0 0, 10px 10px' }, // ลายจุด Polka dot
    ...
    <div className="ig-story-overlay" onClick={() => setIsColorModalOpen(false)} style={{ zIndex: 10000 }}>
      <div className="avatar-modal-box" onClick={(e) => e.stopPropagation()}>
        <div className="avatar-title-wrapper">
          <h2 className="avatar-modal-title">🖌️ Choose Banner Color</h2>
        </div>
